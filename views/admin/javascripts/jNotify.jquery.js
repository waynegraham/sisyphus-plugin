...
@Revison :    	2.1
@Date : 		01/2011
@author: wayne graham updated plugin data to work with Omeka
...
(function($){
	...
	jQuery.jNotify = {
		...
		init:function(msg, options, id) {
			...
			/** Box **/
			if($("#"+id).length == 0)
				$Div = jQuery.jNotify._construct(id, msg);
			...
			if(opts.ShowOverlay && $("#jOverlay").length == 0)
				jQuery.jNotify._showOverlay($Div);
	...
	jNotify = function(msg,options) {
		if(jQuery.jNotify._isReadable('jNotify'))
			jQuery.jNotify.init(msg,options,'jNotify');
	};

	jSuccess = function(msg,options) {
		if(jQuery.jNotify._isReadable('jSuccess'))
			jQuery.jNotify.init(msg,options,'jSuccess');
	};

	jError = function(msg,options) {
		if(jQuery.jNotify._isReadable('jError'))
			jQuery.jNotify.init(msg,options,'jError');
	};
})(jQuery);
